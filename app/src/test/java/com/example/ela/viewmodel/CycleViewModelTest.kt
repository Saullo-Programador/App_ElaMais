...
import com.example.ela.domain.usecase.care.ResetCompletedActionsUseCase
...
class CycleViewModelTest {
    ...
    private val resetCompletedActionsUseCase = mockk<ResetCompletedActionsUseCase>()
    ...
    fun `saveCycle should call useCase and schedule notifications`() = runTest {
        ...
        coEvery { resetCompletedActionsUseCase() } returns Unit

        val viewModel = CycleViewModel(
            getCycleUseCase, saveCycleUseCase, scheduleCycleNotificationsUseCase,
            resetCompletedActionsUseCase ,
        )
